...
def add_data_for_name(name_data, year, rank, name):
    ...
    """
    # Check if the dictionary of this name has been created
    if name in name_data:
        # Check if the key has been created
        if year in name_data[name]:
            # Check if the rank is lower than the original one
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        # assign the first 4 string into the variable(year)
        year = f.read(4)
        next(f)
        for line in f:
            ...
            name1 = name1.strip()
            name2 = name2.strip()
            # assign the information to load into the dictionary(name_data)
